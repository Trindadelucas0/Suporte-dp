...
**IMPORTANTE**: No Render, as variáveis de ambiente são configuradas no painel, **NÃO** através de arquivo `.env`. O arquivo `.env` é apenas para desenvolvimento local.
...
|----------|-----------|------------|
| `SESSION_SECRET` | **RECOMENDADO** - Chave secreta para sessões | Veja instruções abaixo ⚠️ |
...
⚠️ **Nota sobre SESSION_SECRET**: Se não configurar, o sistema gerará automaticamente um secret seguro. Porém, **é altamente recomendado** configurar manualmente para manter o mesmo secret entre reinicializações do servidor.

#### 🔐 Gerando o SESSION_SECRET

**IMPORTANTE**: O `SESSION_SECRET` deve ser uma string aleatória e segura.

**Opção 1 - Gerar localmente (Recomendado):**
...
Você também pode usar qualquer gerador de strings aleatórias seguras.
...
**⚠️ Nota**: Se você não configurar o `SESSION_SECRET`, o sistema gerará automaticamente um secret seguro na primeira inicialização. Porém, isso significa que:
- O secret mudará a cada reinicialização do servidor
- Usuários podem perder suas sessões ao fazer login
- **Recomendamos fortemente configurar manualmente** para manter consistência
...
1. **Logs do Servidor**: 
   ...
   - Se aparecer `⚠️ ATENÇÃO: SESSION_SECRET não foi configurado!`, configure manualmente no Render (veja instruções acima)
...
**Nota**: A partir da versão atual, o sistema gera automaticamente um `SESSION_SECRET` se não estiver configurado, então o deploy não deve mais falhar. Porém, é recomendado configurar manualmente para manter consistência.
