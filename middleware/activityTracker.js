 ...
 * NOTA: lastActivity na sessão é atualizado pelo checkInactivity, não aqui
 * Este middleware apenas atualiza o banco de dados
 */
async function trackActivity(req, res, next) {
  if (req.session && req.session.user && req.session.user.id) {
    try {
      // Atualiza última atividade no banco (não bloqueia a requisição)
      // NOTA: Não atualiza req.session.lastActivity aqui - isso é feito no checkInactivity
      db.query(
...
function checkInactivity(req, res) {
  ...
  if (req.session && req.session.user) {
    ...
    if (!req.session.lastActivity) {
      ...
      // express-session salva automaticamente, não precisa chamar save()
    ...
    // Isso marca que o usuário está ativo agora
    // express-session salva automaticamente, não precisa chamar save()
    req.session.lastActivity = now;
  }
